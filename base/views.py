...
def registerPage(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save(commit=False)  # Freazing the submission for accessing the user
...
def userProfile(request, pk):
    ...
    rooms = user.room_set.all()  # get all the childeren of a specific object by doing the model name and then underscore set
    # _set.all() --> https://stackoverflow.com/questions/25386119/whats-the-difference-between-a-onetoone-manytomany-and-a-foreignkey-field-in-d
...
def deleteMessage(request, pk):
    ...
    if request.method == "POST":
        message.delete()
        current_room = message.room.pk
        print("room", current_room)
        return redirect('room', current_room)
